...
func CreateRecordingAccessToken(id uint, download bool) (string, int64, error) {
	if Config.Jwt.Key == "" {
		return "", 0, errors.New("JWT key is required for recording access")
	}
	expiresAt := time.Now().Add(5 * time.Minute).Unix()
	payload := strconv.FormatUint(uint64(id), 10) + ":" + strconv.FormatInt(expiresAt, 10) + ":" + strconv.FormatBool(download)
...
func VerifyRecordingAccessToken(token string, id uint, download bool) bool {
	...
	if err != nil || uint(tokenId) != id || fields[2] != strconv.FormatBool(download) {
		return false
	}
