...
- `ohoimager/desklink-server:1.4.9`：包含 `hbbs` 与 `hbbr`。
- `ohoimager/desklink-api:1.4.9`：包含 Go API、React 管理端与 Web Client。
...
| 参数 | 默认值 | 说明 |
| --- | --- | --- |
| `DESKLINK_SERVER_IMAGE` | `ohoimager/desklink-server:1.4.9` | ID/Relay 镜像 |
| `DESKLINK_API_IMAGE` | `ohoimager/desklink-api:1.4.9` | API/管理端镜像 |
